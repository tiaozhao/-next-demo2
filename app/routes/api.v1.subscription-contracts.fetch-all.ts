...
import { fetchSubscriptionContractsSchema } from '~/types/subscription-contracts/subscription-contract.schema';
import { ErrorHandler } from '~/lib/errors/error-handler';
import { HttpStatusCode } from '~/lib/errors/base-error';
import { ZodError } from 'zod';

const ROUTE_PATH = '/api/v1/subscription-contracts/fetch-all';

/**
 * POST /api/v1/subscription-contracts/fetch-all
...
export const action = withTracing(
  withCors(async ({ request }: ActionFunctionArgs) => {
    const METHOD = 'subscription-contracts.fetch-all';
    try {
      ...
      try {
        const validatedParams = fetchSubscriptionContractsSchema.parse(params);
        
        // Fetch subscription contracts
        const result = await subscriptionContractService.fetchAll(validatedParams);
