...
import { 
  subscriptionContractApproveSchema 
...
const ROUTE_PATH = '/api/v1/subscription-contracts/approve';

/**
 * POST /api/v1/subscription-contracts/approve
 * Approve a subscription contract
 */
export const action = withTracing(
  withCors(async ({ request }: ActionFunctionArgs) => {
    const METHOD = 'subscription-contracts.approve';
    try {
      ...
      try {
        const validatedData = subscriptionContractApproveSchema.parse(requestBody);
        ...
        // Approve the subscription contract using the validated data
        const result = await subscriptionContractService.approve(validatedData);

        loggerService.info(`${METHOD} handler completed`, {
          status: HttpStatusCode.OK,
          routePath: ROUTE_PATH,
          subscriptionId: validatedData.subscriptionContractId,
          nextOrderDate: result.nextOrderDate,
        });
