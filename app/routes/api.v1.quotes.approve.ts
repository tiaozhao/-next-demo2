...
import { approveQuoteSchema } from '~/types/quotes/quote.schema';
import { loggerService } from '~/lib/logger';
import { ErrorHandler } from '~/lib/errors/error-handler';
import { withTracing } from '~/lib/telemetry/middleware';

const ROUTE_PATH = '/api/v1/quotes/approve';

/**
 * POST /api/v1/quotes/approve
 * Approve a quote that is in Requested status
 */
export const action = withTracing(withCors(async ({ request }: ActionFunctionArgs) => {
  loggerService.info('Quote approve handler started', {
    url: request.url,
  ...
  try {
    ...
    const validatedParams = approveQuoteSchema.safeParse(rawParams);
    ...
    loggerService.info('Approving quote', {
      ...
      actionBy: validatedParams.data.customerId
    });

    const result = await quoteService.approveQuote(validatedParams.data);

    loggerService.info('Quote approve handler completed', {
      status: 200,
      routePath: ROUTE_PATH
    });
...
export const loader = withTracing(withCors(async ({ request }: ActionFunctionArgs) => {
  loggerService.info('Quote approve loader started', {
    method: request.method,
    routePath: ROUTE_PATH
  });
