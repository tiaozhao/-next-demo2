...
import { 
  subscriptionContractDeclineSchema 
...
const ROUTE_PATH = '/api/v1/subscription-contracts/decline';

/**
 * POST /api/v1/subscription-contracts/decline
 * Decline a subscription contract
 */
export const action = withTracing(
  withCors(async ({ request }: ActionFunctionArgs) => {
    const METHOD = 'subscription-contracts.decline';
    try {
      ...
      try {
        const validatedData = subscriptionContractDeclineSchema.parse(requestBody);
        ...
        // Decline the subscription contract using the validated data
        const result = await subscriptionContractService.decline(validatedData);
