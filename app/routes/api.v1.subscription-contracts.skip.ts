...
import { 
  subscriptionContractSkipSchema 
...
const ROUTE_PATH = '/api/v1/subscription-contracts/skip';

/**
 * POST /api/v1/subscription-contracts/skip
 * Skip the current subscription cycle
 */
export const action = withTracing(
  withCors(async ({ request }: ActionFunctionArgs) => {
    const METHOD = 'subscription-contracts.skip';
    try {
      ...
      try {
        const validatedData = subscriptionContractSkipSchema.parse(requestBody);
        ...
        // Skip the subscription cycle using the validated data
        const result = await subscriptionContractService.skip(validatedData);
