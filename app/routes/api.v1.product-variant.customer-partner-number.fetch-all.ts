...
import { storeCompanyMappingQuerySchema } from '~/types/product-variant/store-company-mapping.schema';
import { loggerService } from '~/lib/logger';
import { ErrorHandler } from '~/lib/errors/error-handler';
import { withTracing } from '~/lib/telemetry/middleware';

const ROUTE_PATH = '/api/v1/product-variant/customer-partner-number/fetch-all';

export const action = withTracing(withCors(async ({ request }: ActionFunctionArgs) => {
  loggerService.info('Fetch all handler started', {
    url: request.url,
  ...
  try {
    ...
    const validatedParams = storeCompanyMappingQuerySchema.safeParse(rawParams);
    ...
    const result = await storeCompanyMappingService.fetchAllWithFilters(validatedParams.data);
    return new Response(
...
export const loader = withTracing(withCors(async ({ request }: ActionFunctionArgs) => {
  loggerService.info('Fetch all loader started', {
    method: request.method,
    routePath: ROUTE_PATH
  });
