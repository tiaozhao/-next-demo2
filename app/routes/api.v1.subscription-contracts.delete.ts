...
import { 
  subscriptionContractDeleteSchema 
...
const ROUTE_PATH = '/api/v1/subscription-contracts/delete';

/**
 * POST /api/v1/subscription-contracts/delete
 * Delete a subscription contract by ID
 */
export const action = withTracing(
  withCors(async ({ request }: ActionFunctionArgs) => {
    const METHOD = 'subscription-contracts.delete';
    try {
      ...
      try {
        const validatedData = subscriptionContractDeleteSchema.parse(requestBody);
        ...
        // Delete the subscription contract using the validated data
        const result = await subscriptionContractService.deleteById(validatedData);
