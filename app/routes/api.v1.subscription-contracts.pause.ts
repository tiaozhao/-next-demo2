...
import { 
  subscriptionContractPauseSchema 
...
const ROUTE_PATH = '/api/v1/subscription-contracts/pause';

/**
 * POST /api/v1/subscription-contracts/pause
 * Pause a subscription contract
 */
export const action = withTracing(
  withCors(async ({ request }: ActionFunctionArgs) => {
    const METHOD = 'subscription-contracts.pause';
    try {
      ...
      try {
        const validatedData = subscriptionContractPauseSchema.parse(requestBody);
        ...
        // Pause the subscription contract using the validated data
        const result = await subscriptionContractService.pause(validatedData);
