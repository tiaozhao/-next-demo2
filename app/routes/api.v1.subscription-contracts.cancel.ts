...
import { 
  subscriptionContractCancelSchema 
...
const ROUTE_PATH = '/api/v1/subscription-contracts/cancel';

/**
 * POST /api/v1/subscription-contracts/cancel
 * Cancel a subscription contract
 */
export const action = withTracing(
  withCors(async ({ request }: ActionFunctionArgs) => {
    const METHOD = 'subscription-contracts.cancel';
    try {
      ...
      try {
        const validatedData = subscriptionContractCancelSchema.parse(requestBody);
        ...
        // Cancel the subscription contract using the validated data
        const result = await subscriptionContractService.cancel(validatedData);
